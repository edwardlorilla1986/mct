...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ localization()->getCurrentLocaleDirection() }}">
    <head>
        ...
        <meta name="p:domain_verify" content="0a51912757a8b74b919abb3eee154116"/>
  ...
  @endif
        ...
        @livewireStyles
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-Y0QZF0BF79"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-Y0QZF0BF79');
</script>
    </head>
                              ...
                              <div class="position-relative overflow-hidden text-center bg-light">
                                ...
                                <div class="container position-relative zindex-1">
                                    <div class="col text-center p-lg-5 mx-auto my-5">
                                        ...
                                        <h1 class="display-5 fw-normal">{{ __($page->title) ? __($page->title) : $page->translations[0]->title  }}</h1>
                                        <h2 class="fw-normal">{{ __($page->subtitle) ?  __($page->subtitle) :  $page->translations[0]->subtitle  }}</h2>
                                          ...
                                          <div class="card-body {{ ($general->author_box_status) ? 'pb-0' : ''}}">
                                              @if ( Auth::user() && Auth::user()->is_admin )
                                                <div class="d-flex justify-content-center mb-3">
                                                  <a href="{{ route('admin.pages.translations.edit', $page->translations[0]['id']) }}" class="btn btn-primary">{{ __('Edit Page') }}</a>
                                                </div>
                                              @endif

                                             

                                            {!! !empty($page->description) ? $page->description : (!empty($page->translations) && isset($page->translations[0]) ? $page->translations[0]->description : '') !!}
